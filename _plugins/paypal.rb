...
# Example:
#
# {% paypal 78YFVY22AWW42 "Buy me coffee" %}
#
# Output:
...
module Jekyll
  ...
  class PaypalButton < Liquid::Tag
    ...
    def render(context)
      output = super
      @text ||= context.registers[:site].config['text'] || "Your donations are appreciated!"
      @amount ||= context.registers[:site].config['amount'] || "UNCONFIGURED"
      @action ||=context.registers[:site].config['action'] || "https://www.paypal.com/us/cgi-bin/webscr"
      donate =  %Q{
        <form class="paypalform" action="https://www.paypal.com/cgi-bin/webscr" method="post">
        <input type="hidden" name="cmd" value="_s-xclick">
...
Liquid::Template.register_tag('donate', Jekyll::PaypalButton)
